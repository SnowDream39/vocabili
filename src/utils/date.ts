...
import { type SequentialBoard } from './board'
...
export function issueBefore(date?: DateTime) {
  ...
  return {
    'vocaloid-daily': Math.floor(now.diff(DateTime.local(2024,7,3), 'days').toObject().days!) ,
    'vocaloid-weekly': Math.floor(now.diff(DateTime.local(2024,8,31), 'weeks').toObject().weeks!) ,
    'vocaloid-monthly': Math.floor(now.diff(DateTime.local(2024,7,1), 'months').toObject().months!) ,
  }
}
 ...
 * @return 包括各种刊物的期数
 */
export function issueNow(date?: DateTime): Record<SequentialBoard, number> {
  const now = date || DateTime.local()
  const last = issueBefore(now)
  return {
    'vocaloid-daily': last['vocaloid-daily'] + 1,
    'vocaloid-weekly': last['vocaloid-weekly'] + 1,
    'vocaloid-monthly': last['vocaloid-monthly'] + 1,
  }
}
 ...
 *  获取 issue 对应的开始时间
 */
export function startTimeOf(issue: number, board: SequentialBoard) {
  if (board === 'vocaloid-daily') {
    return DateTime.local(2024,7,2).plus({days: issue})
  } else if (board === 'vocaloid-weekly') {
 ...
 */
export function endTimeOf(issue: number, board: SequentialBoard) {
  if (board === 'vocaloid-daily') {
