...
import SongInfoBlock from '@/components/song/SongInfoBlockV2.vue';
import SongChart from '@/components/chart/SongChartV2.vue';
...
import type { SequentialBoard } from '@/utils/boardv2';
...
async function fetchData(board: SequentialBoard) {
  ...
  chart.index += 1;
  const response = await apiv2.getSongRanking(songId.value, board);
...
async function loadAll(boardId: string) {
  ...
  while (true) {
    chart.index += 1;
    const response = await apiv2.getSongRanking(songId.value, boardId, chart.index, 64);
