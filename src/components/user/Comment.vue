<template>
  <div class="comment-block">
    ...
    <div class="flex justify-between items-center mb-1">
      <span class="font-medium">{{ comment.username || '匿名' }}</span>
      <span class="text-xs">{{ formatTime(comment.created_at) }}</span>
    </div>
    ...
    <!-- 点赞、回复按钮 -->
    <div class="actions text-right text-xs flex flex-row justify-between text-lg">
    ...
    <!-- 嵌套显示回复 -->
    <div v-if="comment.replies && comment.replies.length > 0" class="replies border-l ml-4 mt-2">
      <CommentList
