import { defineConfig, presetWind4 } from 'unocss'
import presetIcons from '@unocss/preset-icons'

export default defineConfig({
  presets: [
    presetIcons(),
    presetWind4({
      dark: 'media',
    }),
  ],
  safelist: [
    'i-material-symbols-thumb-up-outline',
    'i-material-symbols-thumb-up',
    'i-material-symbols-reply',
    'i-material-symbols-reply',
    'i-material-symbols-thumb-down-outline',
    'i-material-symbols-thumb-down',
    'i-material-symbols-share',
    'i-material-symbols-share',
    'border-l-2',
    'border',
  ],
  shortcuts: [
    ['big-title', ['text-2xl', 'font-bold']],
    ['small-title', ['text-xl', 'font-bold']],
    ['glass-button-sm', ['text-sm', 'border-1', 'border-white', 'hover:bg-white/30', 'active:bg-white/40', 'text-shadow-sm', 'text-shadow-white', 'dark:text-shadow-black', 'backdrop-blur-2', 'rounded-md', 'p-1', 'transition-colors', 'cursor-pointer']]
  ],
  content: {
    pipeline: {
      include: ['src/**/*.{vue,ts,js}'],
    },
  },
})
